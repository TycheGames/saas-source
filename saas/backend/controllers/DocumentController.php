...
class DocumentController extends Controller {
	...
	public function actionApi() {
        ...
        foreach ($configs as $config) {
            ...
            foreach ($methods as $method) {
                    ...
                    if ($class == $config['class'] && $actionName == $method->name) {
                        ...
                        if($check_credit == 'credit'){
                            $debugUrl = str_replace(
                                    ...
                                    Yii::$app->request->getHostInfo() . Yii::$app->request->getBaseUrl()
                                    ) . '/' . $debugRoute;
                        }else {
                            $debugUrl = str_replace(
                                            ['backend', 'admin.kdqugou.com'],
                ...
                $label = explode(" ", $actionModel->getTitle());
                $items[] = [
                    'label' => isset($label[0]) ? $label[0] . '<br />' . $actionModel->getRoute() :  $actionModel->getTitle(),
    ...
    public function actionApiDebug($route)
    {
        ...
        $debugUrl = str_replace('backend', 'frontend', Yii::$app->request->getBaseUrl()) . '/' . $route;
    ...
    public function actionEncryptApiDebug($route) {
        ...
        $debugUrl = str_replace('backend', 'frontend', Yii::$app->request->getBaseUrl()) . '/' . $route;
        $encryptUrl = str_replace('backend', 'frontend', Yii::$app->request->getBaseUrl()) .'/'.$controllerId. '/encrypt-test-data';
