 ...
 */
$this->showsubmenu(Yii::T('common', 'Repayment order data'), array(
    ));
?>
...
<?php $form = ActiveForm::begin(['id' => 'search_form', 'method' => 'get', 'action' => $isAllPlatform ? Url::to(['data-stats/day-data-repayment-num-statistics-full-platform']) : Url::to(['data-stats/day-data-repayment-num-statistics']), 'options' => ['style' => 'margin-top:5px;']]); ?>
...
<form name="listform" method="post">
    <table class="tb tb2 fixpadding">
        ...
        <tr class="hover">
            ...
            <td class="td25"><?php echo empty($total_info['t_expire_num_0']) ? '-' : sprintf("%0.2f", (($total_info['t_expire_num_0'] - $total_info['t_repay_num_0']) / $total_info['t_expire_num_0']) * 100) . "%"; ?></td>
            <td class="td25"><?php echo empty($total_info['t_extend_num_0']) ? '-' : $total_info['t_extend_num_0']; ?></td>
            <td class="td25" style="border-right:1px solid #A9A9A9;"><?php echo empty($total_info['expire_num_0']) ? '-' : sprintf("%0.2f", ($total_info['t_extend_num_0'] / ($total_info['expire_num_0'])) * 100) . "%"; ?></td>
            ...
            <td class="td25" style="text-align:center;color:blue"><?php echo empty($total_info['t_expire_num_1']) ? '-' : sprintf("%0.2f", (($total_info['t_expire_num_1'] - $total_info['t_repay_num_1']) / $total_info['t_expire_num_1']) * 100) . "%"; ?></td>
            <td class="td25" style="text-align:center;color:blue"><?php echo empty($total_info['t_extend_num_1']) ? '-' : $total_info['t_extend_num_1']; ?></td>
            <td class="td25" style="text-align:center;color:blue;border-right:1px solid blue;"><?php echo empty($total_info['expire_num_1']) ? '-' : sprintf("%0.2f", ($total_info['t_extend_num_1'] / ($total_info['expire_num_1'])) * 100) . "%"; ?></td>
            ...
            <td class="td25" style="text-align:center;color:red"><?php echo empty($total_info['t_expire_num_2']) ? '-' : sprintf("%0.2f", (($total_info['t_expire_num_2'] - $total_info['t_repay_num_2']) / $total_info['t_expire_num_2']) * 100) . "%"; ?></td>
            <td class="td25" style="text-align:center;color:red"><?php echo empty($total_info['t_extend_num_2']) ? '-' : $total_info['t_extend_num_2']; ?></td>
            <td class="td25" style="text-align:center;color:red;border-right:1px solid red;"><?php echo empty($total_info['expire_num_2']) ? '-' : sprintf("%0.2f", ($total_info['t_extend_num_2'] / ($total_info['expire_num_2'])) * 100) . "%"; ?></td>
        ...
        <?php foreach ($info as $key => $value): ?>
            <tr class="hover"
                ...
                <td class="td25"><?php echo ($value['unix_time_key'] >= $now_date_time) ? '-' : empty($value['expire_num_0']) ? '-' : sprintf("%0.2f", (($value['expire_num_0'] - $value['repay_num_0']) / $value['expire_num_0']) * 100) . "%"; ?></td>
                <td class="td25"><?php echo ($value['unix_time_key'] >= $now_date_time) ? '-' : empty($value['extend_num_0']) ? '-' : $value['extend_num_0']; ?></td>
                <td class="td25" style="border-right:1px solid #A9A9A9;"><?php echo ($value['unix_time_key'] >= $now_date_time) ? '-' : empty($value['expire_num_0']) ? '-' : sprintf("%0.2f",$value['extend_num_0'] / ($value['expire_num_0']) * 100)."%"; ?></td>
                ...
                <td class="td25" style="text-align:center;color:blue"><?php echo ($value['unix_time_key'] >= $now_date_time) ? '-' : empty($value['expire_num_1']) ? '-' : sprintf("%0.2f", (($value['expire_num_1'] - $value['repay_num_1']) / $value['expire_num_1']) * 100) . "%"; ?></td>
                <td class="td25" style="text-align:center;color:blue"><?php echo ($value['unix_time_key'] >= $now_date_time) ? '-' : empty($value['extend_num_1']) ? '-' : $value['extend_num_1']; ?></td>
                <td class="td25" style="text-align:center;color:blue;border-right:1px solid blue;"><?php echo ($value['unix_time_key'] >= $now_date_time) ? '-' : empty($value['expire_num_1']) ? '-' : sprintf("%0.2f",$value['extend_num_1'] / ($value['expire_num_1']) * 100)."%"; ?></td>
                ...
                <td class="td25" style="text-align:center;color:red"><?php echo ($value['unix_time_key'] >= $now_date_time) ? '-' : empty($value['expire_num_2']) ? '-' : sprintf("%0.2f", (($value['expire_num_2'] - $value['repay_num_2']) / $value['expire_num_2']) * 100) . "%"; ?></td>
                <td class="td25" style="text-align:center;color:red"><?php echo ($value['unix_time_key'] >= $now_date_time) ? '-' : empty($value['extend_num_2']) ? '-' : $value['extend_num_2']; ?></td>
                <td class="td25" style="text-align:center;color:red;border-right:1px solid red;"><?php echo ($value['unix_time_key'] >= $now_date_time) ? '-' : empty($value['expire_num_2']) ? '-' : sprintf("%0.2f",$value['extend_num_2'] / ($value['expire_num_2']) * 100)."%"; ?></td>
            </tr>
        <?php endforeach; ?>
