    ...
    <table class="logintb">
        <tr>
            <td class="login-form-wrapper">
                <h3>SAAS ERP -  User Login</h3>
                <p style="color:red;"><?php if ($model->hasErrors()) { $_err = $model->getFirstErrors(); echo array_shift($_err); } ?></p>
                <p class="logintitle">Username：</p>
                <p class="loginform"><input type="text" class="txt user" name="LoginForm[username]" id="LoginForm_username" value="<?= \yii\helpers\Html::encode($model->username); ?>"></p>
                <p class="logintitle">OTP：</p>
                <p class="loginform" style="height:30px;width:300px;">
                    <input type="text" name="LoginForm[phoneCaptcha]" value="<?php echo \yii\helpers\Html::encode($model->phoneCaptcha); ?>" class="txt number" id="loginform-phoneCaptcha" style="width:60px;"/>
