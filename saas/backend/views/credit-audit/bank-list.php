...
$this->showsubmenu('Bank List', array(
));
?>
<?php $form = ActiveForm::begin(['method' => "get", 'action'=>Url::to(['credit-audit/bank-list']), 'options' => ['style' => 'margin-top: 10px;margin-bottom:10px;'],  ]); ?>
...
<?php echo Yii::T('common', 'status') ?>：<?= Html::dropDownList('status', Html::encode(\yii::$app->request->get('status', '')), \common\helpers\CommonHelper::getListT(UserLoanOrder::$order_status_map),['prompt' => 'all']); ?>&nbsp;
...
<table class="tb tb2 fixpadding">
    ...
    <?php foreach ($data_list as $value): ?>
        <tr class="hover">
            ...
            <th><?= Html::encode(Yii::T('common', UserLoanOrder::$order_status_map[$value['status']]) ?? ''); ?></th>
            <th>
                <?php if($value['can_get']): ?>
                <a onclick="javascript:if(!confirmMsg('are you sure get this order!')) return false;" href="<?= Url::to(['credit-audit/to-my-bank-review', 'id' => CommonHelper::idEncryption($value['id'], 'order')]);?>">toMyBankReview</a>
                <?php endif; ?>
            </th>
        </tr>
    <?php endforeach; ?>
