...
$this->showsubmenu('催收员牛信账号绑定', array(
    array(Yii::T('common', 'List'), Url::toRoute('back-end-admin-user/nx-list'), 1),
    array(Yii::T('common', 'Add'), Url::toRoute('back-end-admin-user/nx-add'), 0),
));

?>

    <style>.tb2 th{ font-size: 12px;}</style>
<?php $form = ActiveForm::begin(['method' => "get", 'action'=>Url::to(['back-end-admin-user/nx-list']), 'options' => ['style' => 'margin-top: 10px;margin-bottom:10px;'] ]); ?>
...
<?php $form = ActiveForm::end(); ?>
    ...
    <?php $form = ActiveForm::begin(['id' => 'up', 'method' => "post", 'action'=>Url::to(['back-end-admin-user/nx-batch-add']),'options' => ['enctype' => 'multipart/form-data']]); ?>
    ...
    <table class="tb tb2 fixpadding">
        ...
        <?php foreach ($users as $value): ?>
            <tr class="hover">
                ...
                <th><?php echo Html::encode(\backend\models\AdminNxUser::$status_map[$value['status']]); ?></th>
                <th><?php echo Html::encode(\backend\models\AdminNxUser::$type_map[$value['type']]); ?></th>
                <td>
                    <a href="<?php echo Url::to(['back-end-admin-user/nx-edit', 'id' => $value['id'],'history_url' => $url]); ?>">Edit</a>&nbsp;&nbsp;
                    <a href="<?php echo Url::to(['back-end-admin-user/nx-del', 'id' => $value['id'],'history_url' => $url]); ?>">Delete</a>
                </td>
            </tr>
        <?php endforeach; ?>
    </table>
