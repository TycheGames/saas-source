...
<table class="tb tb2 fixpadding">
	...
	<tr class="hover">
		...
		<td class="td24">
            <a href="<?php echo Url::to(['back-end-admin-user/role-details','role'=> $value->name])?>"><?php echo Yii::T('common', 'member') ?></a>
			<?php if ($value->name != AdminUser::SUPER_ROLE && ($value->groups != AdminUserRole::TYPE_COLLECTION) ): ?>
			<a href="<?php echo Url::to(['back-end-admin-user/role-edit', 'id' => $value->id]); ?>"><?php echo Yii::T('common', 'edit') ?></a>
			<a onclick="return confirmMsg('Are you sure you want to delete it? \nAfter deletion, the administrator corresponding to this role will lose permission.');" href="<?php echo Url::to(['back-end-admin-user/role-delete', 'id' => $value->id]); ?>"><?php echo Yii::T('common', 'del') ?></a>
			<?php else: ?>
            <?php endif; ?>
		</td>
	</tr>
