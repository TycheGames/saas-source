...
$this->showsubmenu(Yii::T('common', 'Daily borrowing data (principal - user structure)'));
?>
<style>
...
<?php $form = ActiveForm::begin(['id' => 'search_form','method'=>'get','action'=> Url::to(['data-stats-full-platform/daily-data-user-structure']), 'options' => ['style' => 'margin-top:5px;']]); ?>
    <?php
    echo Html::dropDownList('search_date', Html::encode($search_date), array(1=>Yii::T('common', 'Loan date')));
    ?>
    ...
    <form name="listform" method="post">
        <table class="tb tb2 fixpadding">
            <tr class="header">
                ...
                <th colspan="3" style="color:blue;text-align: center;border-right:1px solid blue;"><?php echo Yii::T('common', 'All new self new') ?></th>
                <th colspan="3" style="color:red;text-align: center;border-right:1px solid red;"><?php echo Yii::T('common', 'All old self new') ?></th>
                <th colspan="3" style="color:blue;text-align: center;border-right:1px solid blue;"><?php echo Yii::T('common', 'All old self old') ?></th>
            </tr>
            <tr class="header">
            ...
            <tr>
                <?php
                $total_all = $total_loan_num_new + $total_loan_num_old + $total_loan_num_all_old_loan_new;
                $total_new_pre = (!empty($total_all)) ? round(($total_loan_num_new/$total_all)*100) : 0;
                $total_old_pre = (!empty($total_all)) ? round(($total_loan_num_old/$total_all)*100) : 0;
                $total_all_old_loan_new_pre = (!empty($total_all)) ? round(($total_loan_num_all_old_loan_new/$total_all)*100) : 0;
                ?>
                ...
                <th style="border-right:1px solid #A9A9A9;"><?php echo "<span style='color:blue'>".$total_new_pre."</span>" . " : " . "<span style='color:red'>".$total_all_old_loan_new_pre."</span>" . " : " . "<span style='color:blue'>".$total_old_pre."</span>"; ?></th>
                ...
                <th style="text-align:center;color:red"><?php echo $total_loan_num_all_old_loan_new ?></th>
                <th style="text-align:center;color:red"><?php echo number_format($total_loan_money_all_old_loan_new/100) ?></th>
                <th style="text-align:center;color:red;border-right:1px solid red;"><?php echo ($total_loan_num_all_old_loan_new>0)?sprintf("%0.2f",($total_loan_money_all_old_loan_new/100)/$total_loan_num_all_old_loan_new):0; ?></th>
                <th style="text-align:center;color:blue"><?php echo $total_loan_num_old ?></th>
                <th style="text-align:center;color:blue"><?php echo number_format($total_loan_money_old/100) ?></th>
                <th style="text-align:center;color:blue;border-right:1px solid blue;"><?php echo ($total_loan_num_old>0)?sprintf("%0.2f",($total_loan_money_old/100)/$total_loan_num_old):0; ?></th>
            </tr>
            <?php foreach ($data as $value): ?>
                <tr class="hover" style="<?php echo date('w', $value['date_time']) == 0 || date('w', $value['date_time']) == 6?'background:#deeffa':'';?>">
                    <?php
                        ...
                        $loan_num_all_old_loan_new = $rate*$value['loan_num_all_old_loan_new'];
                        $total = $loan_num_new + $loan_num_old + $loan_num_all_old_loan_new;
                        $new_pre = (!empty($total)) ? round(($loan_num_new/$total)*100) : 0;
                        $old_pre = (!empty($total)) ? round(($loan_num_old/$total)*100) : 0;
                        $all_old_loan_new = (!empty($total)) ? round(($loan_num_all_old_loan_new/$total)*100) : 0;
                        ...
                        $loan_money_all_old_loan_new = $rate*$value['loan_money_all_old_loan_new']/100;
                    ?>
                    <!-- 借款信息 -->
                    ...
                    <td class="td25" style="border-right:1px solid #A9A9A9;"><?php echo "<span style='color:blue'>".$new_pre."</span>" . " : " . "<span style='color:red'>".$all_old_loan_new."</span>" . " : " . "<span style='color:blue'>".$old_pre."</span>"; ?></td>

                    <!-- 新用户 -->
                    <td class="td25" style="text-align:center;color:blue"><?php echo $loan_num_new; ?></td>
                    ...
                    <!-- 全老本新用户 -->
                    <td class="td25" style="text-align:center;color:red;"><?php echo $loan_num_all_old_loan_new; ?></td>
                    <td class="td25" style="text-align:center;color:red;"><?php echo number_format(sprintf("%0.2f",$loan_money_all_old_loan_new)); ?></td>
                    <td class="td25" style="text-align:center;border-right:1px solid red;color:red;"><?php echo ($loan_num_all_old_loan_new>0)?sprintf("%0.2f",$loan_money_all_old_loan_new/$loan_num_all_old_loan_new):0; ?></td>

                    <!-- 老用户 -->
                    <td class="td25" style="text-align:center;color:blue;"><?php echo $loan_num_old; ?></td>
                    <td class="td25" style="text-align:center;color:blue;"><?php echo number_format(sprintf("%0.2f",$loan_money_old)); ?></td>
                    <td class="td25" style="text-align:center;border-right:1px solid blue;color:blue;"><?php echo ($loan_num_old>0)?sprintf("%0.2f",$loan_money_old/$loan_num_old):0; ?></td>
                </tr>
            <?php endforeach; ?>
        </table>
        <?php if (empty($data)): ?>
