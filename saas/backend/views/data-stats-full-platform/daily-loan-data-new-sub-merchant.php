 ...
 */
$this->showsubmenu(Yii::T('common', 'Daily repayment order data(user structure)').'sub', array(
    ));
?>
...
<?php $form = ActiveForm::begin(['id' => 'search_form', 'method' => 'get', 'action' => Url::to(['data-stats-full-platform/day-data-repayment-num-statistics-user-structure-sub-merchant']), 'options' => ['style' => 'margin-top:5px;']]); ?>
...
<?php if($isNotMerchantAdmin): ?>
    <?php echo Yii::T('common', 'merchant') ?>：<?php  echo \yii\helpers\Html::dropDownList('merchant_id', ArrayHelper::htmlEncode(Yii::$app->getRequest()->get('merchant_id', [])),
        $merchantList,['class' => 'form-control merchant-select', 'multiple' => 'multiple', 'id' => 'merchant']); ?>&nbsp;
<?php endif;?>
...
<form name="listform" method="post">
    <table class="tb tb2 fixpadding">
        <tr class="header">
            <th style="text-align:center;border-right:1px solid #A9A9A9;"><?php echo Yii::T('common', 'Merchant name') ?></th>
        ...
        <?php foreach ($data as $merchantId => $info): ?>
        <tr class="hover">
            <!-- 借款信息 -->
            <td style="border-right:1px solid #A9A9A9;" ><?php echo $merchantList[$merchantId] ?? '' ?></td>
            <td style="border-right:1px solid #A9A9A9;"><?php echo Yii::T('common', 'Summary information') ?></td>

            <!-- 所有用户 -->
            <td class="td25"><?php echo isset($total_info[$merchantId]['expire_num_0']) ? $total_info[$merchantId]['expire_num_0'] : 0; ?></td>
            <td class="td25"><?php echo $total_info[$merchantId]['repay_zc_num_0'] ?? 0; ?></a></td>
            <td class="td25"><?php echo isset($total_info[$merchantId]['repay_num_0']) ? $total_info[$merchantId]['repay_num_0'] : 0; ?></td>
            <td class="td25"><?php echo empty($total_info[$merchantId]['t_expire_num_0']) ? '-' : sprintf("%0.2f", ($total_info[$merchantId]['t_expire_num_0'] - $total_info[$merchantId]['t_repay_zc_num_0']) / $total_info[$merchantId]['t_expire_num_0'] * 100) . "%"; ?></td>
            <td class="td25"><?php echo empty($total_info[$merchantId]['expire_num_0']) ? '-' : sprintf("%0.2f", ($total_info[$merchantId]['repay_num_0'] / $total_info[$merchantId]['expire_num_0']) * 100) . "%"; ?></td>
            <td class="td25"><?php echo (isset($total_info[$merchantId]['t_expire_num_0']) && isset($total_info[$merchantId]['t_repay_num_0'])) ? $total_info[$merchantId]['t_expire_num_0'] - $total_info[$merchantId]['t_repay_num_0'] : '-'; ?></td>
            <td class="td25" style="border-right:1px solid #A9A9A9;"><?php echo empty($total_info[$merchantId]['t_expire_num_0']) ? '-' : sprintf("%0.2f", (($total_info[$merchantId]['t_expire_num_0'] - $total_info[$merchantId]['t_repay_num_0']) / $total_info[$merchantId]['t_expire_num_0']) * 100) . "%"; ?></td>

            <td class="td25"
                style="text-align:center;color:blue"><?php echo isset($total_info[$merchantId]['expire_num_1']) ? $total_info[$merchantId]['expire_num_1'] : 0; ?></td>
            <td class="td25"
                style="text-align:center;color:blue"><?php echo empty($total_info[$merchantId]['t_expire_num_1']) ? '0%' : sprintf("%0.2f", ($total_info[$merchantId]['t_expire_num_1'] - $total_info[$merchantId]['t_repay_zc_num_1']) / $total_info[$merchantId]['t_expire_num_1'] * 100) . "%"; ?></td>
            <td class="td25"
                style="text-align:center;color:blue"><?php echo empty($total_info[$merchantId]['expire_num_1']) ? '-' : sprintf("%0.2f", ($total_info[$merchantId]['repay_num_1'] / $total_info[$merchantId]['expire_num_1']) * 100) . "%"; ?></td>
            <td class="td25"
                style="text-align:center;color:blue"><?php echo((isset($total_info[$merchantId]['t_expire_num_1']) && isset($total_info[$merchantId]['t_repay_num_1'])) ? $total_info[$merchantId]['t_expire_num_1'] - $total_info[$merchantId]['t_repay_num_1'] : '-'); ?></td>
            <td class="td25" style="text-align:center;color:blue;border-right:1px solid blue;"><?php echo empty($total_info[$merchantId]['t_expire_num_1']) ? '-' : sprintf("%0.2f", (($total_info[$merchantId]['t_expire_num_1'] - $total_info[$merchantId]['t_repay_num_1']) / $total_info[$merchantId]['t_expire_num_1']) * 100) . "%"; ?></td>

            <td class="td25"
                style="text-align:center;color:red"><?php echo isset($total_info[$merchantId]['expire_num_2']) ? $total_info[$merchantId]['expire_num_2'] : 0; ?></td>
            <td class="td25"
                style="text-align:center;color:red"><?php echo empty($total_info[$merchantId]['t_expire_num_2']) ? '0%' : sprintf("%0.2f", ($total_info[$merchantId]['t_expire_num_2'] - $total_info[$merchantId]['t_repay_zc_num_2']) / $total_info[$merchantId]['t_expire_num_2'] * 100) . "%"; ?></td>
            <td class="td25"
                style="text-align:center;color:red"><?php echo empty($total_info[$merchantId]['expire_num_2']) ? '-' : sprintf("%0.2f", ($total_info[$merchantId]['repay_num_2'] / $total_info[$merchantId]['expire_num_2']) * 100) . "%"; ?></td>
            <td class="td25"
                style="text-align:center;color:red"><?php echo((isset($total_info[$merchantId]['t_expire_num_2']) && isset($total_info[$merchantId]['t_repay_num_2'])) ? $total_info[$merchantId]['t_expire_num_2'] - $total_info[$merchantId]['t_repay_num_2'] : '-'); ?></td>
            <td class="td25" style="text-align:center;color:red;border-right:1px solid red;"><?php echo empty($total_info[$merchantId]['t_expire_num_2']) ? '-' : sprintf("%0.2f", (($total_info[$merchantId]['t_expire_num_2'] - $total_info[$merchantId]['t_repay_num_2']) / $total_info[$merchantId]['t_expire_num_2']) * 100) . "%"; ?></td>

            <td class="td25"
                style="text-align:center;color:blue"><?php echo isset($total_info[$merchantId]['expire_num_3']) ? $total_info[$merchantId]['expire_num_3'] : 0; ?></td>
            <td class="td25"
                style="text-align:center;color:blue"><?php echo empty($total_info[$merchantId]['t_expire_num_3']) ? '0%' : sprintf("%0.2f", ($total_info[$merchantId]['t_expire_num_3'] - $total_info[$merchantId]['t_repay_zc_num_3']) / $total_info[$merchantId]['t_expire_num_3'] * 100) . "%"; ?></td>
            <td class="td25"
                style="text-align:center;color:blue"><?php echo empty($total_info[$merchantId]['expire_num_3']) ? '-' : sprintf("%0.2f", ($total_info[$merchantId]['repay_num_3'] / $total_info[$merchantId]['expire_num_3']) * 100) . "%"; ?></td>
            <td class="td25"
                style="text-align:center;color:blue"><?php echo((isset($total_info[$merchantId]['t_expire_num_3']) && isset($total_info[$merchantId]['t_repay_num_3'])) ? $total_info[$merchantId]['t_expire_num_3'] - $total_info[$merchantId]['t_repay_num_3'] : '-'); ?></td>
            <td class="td25" style="text-align:center;color:blue;border-right:1px solid blue;"><?php echo empty($total_info[$merchantId]['t_expire_num_3']) ? '-' : sprintf("%0.2f", (($total_info[$merchantId]['t_expire_num_3'] - $total_info[$merchantId]['t_repay_num_3']) / $total_info[$merchantId]['t_expire_num_3']) * 100) . "%"; ?></td>
        </tr>
