...
namespace callcenter\components;
...
class View extends \yii\web\View {
     ...
     * @param string $smallmenuKey		对应三级菜单配置中的key
     */
    public function shownav($topmenuKey = '', $menuKey = '',$smallmenuKey = '') {
        ...
        $smallmenuName = !empty($menu[$topmenuKey][$smallmenuKey]) ? $menu[$topmenuKey][$smallmenuKey][0] : '';
        $title = $topmenuName . ($menuName ? '&nbsp;&raquo;&nbsp;' . $menuName : '') . ($smallmenuName ? '&nbsp;&raquo;&nbsp;' . $smallmenuName : '');
...
EOT;
    ...
    /*
     * Nav wrapper funciton...
     * @param string $nav
     */

    public function navWrapper($nav, $menu = [], $title = '') {
        $_cfg = CommonHelper::getNavConfig($nav);
        $this->shownav($nav);
        $this->title = empty($title) ? $_cfg[0] : $title;
        $this->showsubmenu($this->title, $menu);
    }
