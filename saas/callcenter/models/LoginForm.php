...
class LoginForm extends Model {
    ...
    public function validatePhoneCaptcha($attribute, $params) {
        if (empty($this->errors)) {
            ...
            if(!$service->validatePhoneCaptchaAdmin($user->phone, $this->phoneCaptcha, CaptchaService::TYPE_ADMIN_CS_LOGIN))
            {
    ...
    public function login() {
        if ($this->validate()) {
            ...
            $lockKey = sprintf('%s:%s:%s', RedisQueue::USER_LOGIN_PC_LOCK , 'assist', $this->getUser()->id);
            $listKey = sprintf('%s:%s:%s', RedisQueue::LIST_USER_PC_SESSION , 'assist', $this->getUser()->id);
