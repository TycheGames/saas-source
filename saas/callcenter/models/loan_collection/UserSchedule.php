...
class UserSchedule extends ActiveRecord 
{
            ...
            foreach ($scheduleLists as $key => $schedule) {
              /*  if($schedule['group_id'] == LoanCollection::GROUP_S_TWO && $day_today > self::SECOND_DAY && $day_today <= self::ELEVENTH_DAY){
                ...
                foreach ($groups as $key => $group) {
                   $userList = LoanCollection::groupUser($group, $schedule['company_id']);
 ...
 /**
     ...
     *返回分配规则信息
     */
    public static function schedule_lists($offset= '' , $limit = '',$condition = ''){
        ...
        if(!empty($list)){
            ...
            foreach ($list as $key => $item) {
                ...
                $list[$key]['company_title'] = isset($companies[$item['company_id']]) ? $companies[$item['company_id']]['real_title'].' (id:'.$item['company_id'].')' : '未知公司';
                $list[$key]['group_title'] = isset($groups[$item['group_id']]) ? $groups[$item['group_id']] : '未知分组';
                $group_users = LoanCollection::groupUser($item['group_id'], $item['company_id']);
