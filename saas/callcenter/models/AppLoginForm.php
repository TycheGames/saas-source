...
class AppLoginForm extends Model {
    ...
    public function validatePassword($attribute, $params)
    {
        if (empty($this->errors)) {
            ...
            if($user->login_app == AdminUser::CAN_NOT_LOGIN_APP){
                $this->addError($attribute, 'System is under maintenance');
            }
        }
    }
    ...
    public function validatePhoneCaptcha($attribute, $params) {
        if (empty($this->errors)) {
            ...
            if (!$service->validatePhoneCaptchaAdmin($user->phone, $this->otp, CaptchaService::TYPE_ADMIN_CS_LOGIN)) {
                $message = AdminUser::errorAndLock($user);
                if($message)
                {
                    $this->addError($attribute, $message);
    ...
    public function login() {
        if ($this->validate()) {
            ...
            $lockKey = sprintf('%s:%s:%s', RedisQueue::USER_LOGIN_MOBILE_LOCK , 'assist', $this->getUser()->id);
            $listKey = sprintf('%s:%s:%s', RedisQueue::LIST_USER_MOBILE_SESSION , 'assist', $this->getUser()->id);
        ...
        yii::warning("username:{$this->username} login failed, password is {$this->password}, phoneCaptcha is {$this->otp}", 'callcenter_login_log');
        return false;
    }
