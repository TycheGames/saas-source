...
$resetRoleUrl = Url::toRoute('main/reset-role');
$chLanguage = Url::toRoute('main/language');

$newNum = '';
if($showMessage){
    $csrf = Yii::$app->request->getCsrfToken();
    $message = 'message';
    $newNum = $showMessage['num'] > 0 ? ('('.$showMessage['num'].')') : '';
    $html = "
    <script>
      var int = setInterval(function(){
              ...
              request.open('POST','/index.php?r=main/new-message',true);
              //第四部 发送请求出发下面这个函数
                                ...
                                if(typeof res == 'object' && res ){
                                    if(res.code == 0){
                                        if(res.num == '0'){
                                            ...
                                            document.title = 'Collection system'
                                        }else{
                                            document.getElementById('msg_count').innerHTML = '('+res.num+')';
                                            document.title = res.num +' new messages';
                                            
                                            if(res.have_new){
                                                if(confirm('You have a new message, if you view it now, please click OK')){
                                                     window.parent.main .location.href= 'index.php?r=user-collection/my-message-list'; 
                                                }      
                                            }
                                        }
                                    }else{
...
<link rel="shortcut icon" href="{$BASEURL}/image/xiao.ico" type="image/x-icon" />
...
if($nx_admin){
    echo <<<EOT
        <p>hello，<em>{$name}</em>({$phone})&nbsp; <em>NXPhone</em>:{$nx_admin} [<a href="{$logoutUrl}" target="_top">quit</a>]</em> [<a href="{$resetRoleUrl}" target="_top">refresh</a>] [<a href="{$chLanguage}">changeLanguage</a>]</p>
EOT;
}else{
    echo <<<EOT
        <p>hello，<em>{$name}</em>({$phone}) [<a href="{$logoutUrl}" target="_top">quit</a>]</em> [<a href="{$resetRoleUrl}" target="_top">refresh</a>] [<a href="{$chLanguage}">changeLanguage</a>]</p>
EOT;
}

echo <<<EOT
&nbsp;
<a href="/index.php?r=user-collection/my-message-list" target="main">{$message}</a>&nbsp;<span id='msg_count' style="color: red">$newNum</span>
</div>
$html
<div class="navbg"></div>
<div class="nav">
