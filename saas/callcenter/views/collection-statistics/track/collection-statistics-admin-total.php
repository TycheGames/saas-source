...
?>
<?php $form = ActiveForm::begin(['id' => 'search_form','method'=>'get', 'action'=>Url::to(['collection-statistics/loan-collection-admin-total']), 'options' => ['style' => 'margin-top:5px;']]); ?>
...
<!--<?php echo Yii::T('common', 'Order group') ?>：<?/*=Html::dropDownList('order_level',Yii::$app->getRequest()->get('order_level', 0),LoanCollection::group(),array('prompt' => Yii::T('common', 'All Group')));*/?>&nbsp;-->
...
<form name="listform" method="post">
    <table class="tb tb2 fixpadding">
        ...
        <?php foreach ($loan_collection_statistics as $value): ?>
                <tr class="hover">
                  <!--  <td ><?php /*echo isset($outsides[$value['outside_id']])?$outsides[$value['outside_id']]:'--'; */?></td>-->
                    <td ><?php echo $outsides[$value['outside_id']]; ?></td>
                    <?php if (!empty($arrMerchant)): ?>
                        <td>
                            <?php echo Html::encode(!empty($arrMerchant[$value['merchant_id']]) ? $arrMerchant[$value['merchant_id']] : ''); ?>
                        </td>
                    <?php endif; ?>
                    <td ><?php echo LoanCollectionOrder::$level[$value['loan_group']]; ?></td>
                    <td ><?php echo isset(LoanCollectionOrder::$level[$value['order_level']])?LoanCollectionOrder::$level[$value['order_level']]:'';?></td>
                    ...
                    <td ><?php echo Html::encode($value['oneday_total']); ?></td><!--首日完成单数-->
