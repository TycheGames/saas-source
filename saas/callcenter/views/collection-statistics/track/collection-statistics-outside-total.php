...
?>
<?php $form = ActiveForm::begin(['id' => 'search_form','method'=>'get', 'action'=>Url::to(['collection-statistics/loan-collection-outside-total']),'options' => ['style' => 'margin-top:5px;']]); ?>
...
<form name="listform" method="post">
    <table class="tb tb2 fixpadding">
        ...
        <?php if(!empty($loan_group) || !empty($order_level) || !empty($group_game)):?>
            <tr class="header">
                <th ><?php echo Yii::T('common', 'summary') ?></th>
                <th ></th>
                <th ></th>
                <th ><?php echo Html::encode(number_format($result['total_all_money']/100,2));?></th>
                <th ><?php echo Html::encode(number_format($result['total_yes_money']/100,2));?></th>
                <th ><?php echo Html::encode(number_format($result['total_not_money']/100,2));?></th>
                <th ><?php echo Html::encode($result['total_all_total']);?></th>
                <th ><?php echo Html::encode($result['total_yes_total']);?></th>
                <th ><?php echo Html::encode(sprintf("%.2f", $result['finish_fee']*100).'%'); ?></th>
                <th ><?php echo Html::encode(sprintf("%.2f", $result['oneday_money_fee']*100).'%'); ?></th>
                <th ><?php echo Html::encode(number_format($result['total_all_late']/100,2));?></th>
                <th ><?php echo Html::encode(number_format($result['total_yes_late']/100,2));?></th>
                <th ><?php echo Html::encode(sprintf("%.2f", $result['late_fee']*100).'%'); ?></th>
                <th ><?php echo Html::encode($result['total_operate_total']);?></th>
            </tr>
        <?php endif;?>
        <?php foreach ($outside_data as $value): ?>
                        <tr class="hover">
                            ...
                            <td ><?php echo Html::encode(!empty($value['today_finish_late_fee'])?sprintf("%.2f", number_format(($value['finish_late_fee']/$value['today_finish_late_fee']),4)*100).'%':'--'); ?></td>
