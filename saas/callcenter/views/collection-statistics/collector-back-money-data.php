...
$this->shownav('manage', 'menu_collector_back_money_data');
$this->showsubmenu(Yii::T('common', 'Collector day completion amount (including partial repayment amount)'));
?>
...
<?php $form = ActiveForm::begin(['id' => 'search_form','method'=>'get', 'action'=>Url::to(['collection-statistics/collector-back-money-data2']),'options' => ['style' => 'margin-top:5px;']]); ?>
...
<form name="listform" method="post">
    <table class="tb tb2 fixpadding">
        <tr class="header">
            ...
            <?php endif; ?>
            <th ><a id="back_money"><?php echo Yii::T('common', 'Recall amount (including partial repayment)') ?></a></th>
            <th ><a id="delay_order_count"><?php echo Yii::T('common', 'Delay order count') ?></a></th>
            <th ><a id="delay_money"><?php echo Yii::T('common', 'Delay money') ?></a></th>
            <th ><a id="extend_order_count"><?php echo Yii::T('common', 'Extend order count') ?></a></th>
            <th ><a id="extend_money"><?php echo Yii::T('common', 'Extend money') ?></a></th>
            <th ><a id="finish_order_count"><?php echo Yii::T('common', 'Finish order count') ?></a></th>
        </tr>
        <?php foreach ($totalData as $value): ?>
            <tr <?= ($value['date'] != '汇总') ?'class="hover date-data" hidden' : 'class="hover" onclick= "showDateData()"';?>>
                ...
                <?php if ($setRealNameCollectionAdmin): ?>
                    <td style='<?= ($value['date'] == '汇总') ?"color:blue;font-weight:bold" : "color:red;";?>'>--</td>
                <?php endif; ?>
                <td style='<?= ($value['date'] == '汇总') ?"color:blue;font-weight:bold" : "color:red;";?>'><?php echo Html::encode(number_format($value['back_money'] / 100,2));?></td>
                <td style='<?= ($value['date'] == '汇总') ?"color:blue;font-weight:bold" : "color:red;";?>'><?php echo Html::encode($value['delay_order_count']);?></td>
                <td style='<?= ($value['date'] == '汇总') ?"color:blue;font-weight:bold" : "color:red;";?>'><?php echo Html::encode(number_format($value['delay_money'] / 100,2));?></td>
                <td style='<?= ($value['date'] == '汇总') ?"color:blue;font-weight:bold" : "color:red;";?>'><?php echo Html::encode($value['extend_order_count']);?></td>
                <td style='<?= ($value['date'] == '汇总') ?"color:blue;font-weight:bold" : "color:red;";?>'><?php echo Html::encode(number_format($value['extend_money'] / 100,2));?></td>
                <td style='<?= ($value['date'] == '汇总') ?"color:blue;font-weight:bold" : "color:red;";?>'><?php echo Html::encode($value['finish_order_count']);?></td>
            </tr>
        <?php endforeach; ?>
        <?php foreach ($data as $key => $value): ?>
            <tr class="hover">
                ...
                <?php endif; ?>
                <td ><?php echo Html::encode(number_format($value['back_money'] / 100,2));?></td>
                <td ><?php echo Html::encode($value['delay_order_count']);?></td>
                <td ><?php echo Html::encode(number_format($value['delay_money'] / 100,2));?></td>
                <td ><?php echo Html::encode($value['extend_order_count']);?></td>
                <td ><?php echo Html::encode(number_format($value['extend_money'] / 100,2));?></td>
                <td ><?php echo Html::encode($value['finish_order_count']);?></td>
            </tr>
        <?php endforeach; ?>
    </table>
    <?php if (empty($data)): ?>
...
<script>
    ...
    $('#back_money').click(function(){
        $('#search_form').append('<input type="hidden" name="btn_sort" value="back_money">')
