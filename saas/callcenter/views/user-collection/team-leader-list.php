...
$this->showsubmenu('', array(
    array('team leader list ', Url::toRoute('user-collection/team-leader-list'), 1),
    array('add team leader', Url::toRoute(['user-collection/team-leader-add']),0),
...
team ：<?php echo Html::dropDownList('group_game', Html::encode(Yii::$app->getRequest()->get('group_game', '')), AdminUser::$group_games,['prompt' => '-all team-','id' => 'group_game']); ?>&nbsp;
<?php if($is_self):?>
...
<table class="tb tb2 fixpadding">
    ...
    <?php foreach ($users as $value): ?>
        <tr class="hover">
            ...
            <th><?php echo Html::encode((AdminUser::$group_games[$value['group_game']] ?? '--') . ($value['alias'] ? ':'.$value['alias'] :''));  ?></th>
            <?php if($is_self):?>
            ...
            <td>
                <a href="<?php echo Url::to(['user-collection/team-leader-edit', 'id' => $value['id'],'page'=>$pages->page,'group'=>Yii::$app->getRequest()->get('group', ''),'outside'=>Yii::$app->getRequest()->get('outside', ''),'status'=>Yii::$app->getRequest()->get('status', '')]); ?>">Edit</a>
                <a href="<?php echo Html::encode(Url::to(['user-collection/set-small-team-leader-deputy', 'id' => $value['id']])); ?>">Set Deputy</a>
                <a style="color: red;" onclick="return confirm('Operation irreversible, confirm delete？');" href="<?php echo Url::to(['user-collection/team-leader-del', 'id' => $value['id'],'page'=>$pages->page,'group'=>Yii::$app->getRequest()->get('group', ''),'outside'=>Yii::$app->getRequest()->get('outside', ''),'status'=>Yii::$app->getRequest()->get('status', '')]); ?>">Delete</a>
            </td>
        </tr>
    <?php endforeach; ?>
</table>
