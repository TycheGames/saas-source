...
class Lock {
    ...
    // Lock List
    const LOCK_REG_GET_CODE = "user:reg:get:code";  // 注册
    const LOCK_ALLPY_ORDER_GET_CODE = 'user:apply:order:get:code';
    ...
    public static function lockCode($prefix_key, $rules) { // 设备黑名单列表
        ...
        $phone_white_list = [13818589827,'18601780292'];
        foreach ($rules as $rule_key => $rule) {
            //手机白名单
            if($rule_key == 'phone' && in_array($rule, $phone_white_list)){
                return true;
            }
            // 设备黑名单
