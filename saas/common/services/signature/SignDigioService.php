...
namespace common\services\signature;

use common\models\user\LoanPerson;
use common\models\user\UserSignature;
...
class SignDigioService extends BaseService
{
    protected $domain, $clinetId, $clientSecret;

    public function init()
    {
        if(YII_ENV_PROD){
            $this->domain = 'https://api.digio.in';
            $this->clinetId = 'AI5KFDA1FFTEJHFPCPZYNOQ477Q6ZXYW';
            $this->clientSecret = '9P5DOXNPYRGL61G6MJURM74PXZKTOFU7';
        }else{
            $this->domain = 'https://ext.digio.in:444';
            $this->clinetId = 'AI5KFDA1FFTEJHFPCPZYNOQ477Q6ZXYW';
            $this->clientSecret = '9P5DOXNPYRGL61G6MJURM74PXZKTOFU7';
        }
     ...
     * @return bool
     */
    public function loanSignature($userId)
    {
        //todo 暂时停用电子签名
        $this->setResult([
            'isSignature' => false
        ...
        $result = $this->uploadPdf($loanPerson->phone, $loanPerson->aadhaar_number);
        $model = new UserSignature();
        $model->user_id = $loanPerson->id;
        $model->content = $result;

        $result = json_decode($result,true);
        $model->document_id = $result['id'];
        $model->status = UserSignature::STATUS_DEFAULT;
        if($model->save()){
            $this->setResult([
                'documentId' => $result['id'],
                'logoUrl' => Yii::$app->request->hostInfo . Yii::$app->request->baseUrl . '/logo.png',
                'aadhaarNumber' => $loanPerson->aadhaar_number
            ]);
            return true;
        }else{
            return false;
    ...
    public function uploadPdf($phone, $addhaar)
    {
        $url = $this->domain . '/v2/client/document/uploadpdf';
...
//                'signature_type' => 'electronic'
        ...
        $client = new Client([
//            RequestOptions::PROXY => '192.168.50.245:8888',
            RequestOptions::TIMEOUT => 60,
            RequestOptions::AUTH => [$this->clinetId,$this->clientSecret]
        ]);
