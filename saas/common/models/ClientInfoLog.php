...
class ClientInfoLog extends ActiveRecord
{
    ...
    {
        $loanPerson = LoanPerson::find()->select(['merchant_id'])->where([ 'id' => $userId])->one();
        $model = new self();
        $model->user_id = $userId;
        $model->merchant_id = $loanPerson->merchant_id;
        $model->event = $event;
        ...
        $model->ip = $params['ip'] ?? '';
        $r = $model->save();
        if($event == self::EVENT_LOGIN && $r){
            RedisQueue::push([RedisQueue::PUSH_USER_LOGIN_DATA, $model->id]);
        }
        return $r;
