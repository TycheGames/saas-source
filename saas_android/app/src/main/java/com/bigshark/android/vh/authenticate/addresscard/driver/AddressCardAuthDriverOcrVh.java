 ...
 */
public class AddressCardAuthDriverOcrVh extends DisplayBaseVh<View, Void> {
    ...
    public AddressCardAuthDriverOcrVh(AddressCardAuthActivity activity, MediaStoreCompat mMediaStoreCompat, View root, Callback mCallback) {
        super(activity);
    ...
    protected void bindViews() {
        super.bindViews();
        mFrontRoot = findViewById(R.id.authenticate_address_driver_ocr_front_root);
        mFrontPicImage = findViewById(R.id.authenticate_address_driver_ocr_front_pic);
        mFrontStatusImage = findViewById(R.id.authenticate_address_driver_ocr_front_status);

        mBackRoot = findViewById(R.id.authenticate_address_driver_ocr_back_root);
        mBackPicImage = findViewById(R.id.authenticate_address_driver_ocr_back_pic);
        mBackStatusImage = findViewById(R.id.authenticate_address_driver_ocr_back_status);
    }
    ...
    private void pickPhoto(final boolean isFront) {
        ...
        String fileTagName = "address_driver_" + (isFront ? "front" : "back");
        ...
        File driverFile = new File(mAddressCardAuthActivity.getExternalFilesDir(Environment.DIRECTORY_PICTURES), fileName);

        mMediaStoreCompat.dispatchCaptureIntent(mAddressCardAuthActivity, driverFile, RequestCodeType.DRIVER);
    }


    public boolean onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode != RequestCodeType.DRIVER) {
            return false;
        }
            ...
            if (mFrontFilePath != null && mBackFilePath != null) {
                ...
                HttpSender.post(new CommonResponsePendingCallback<UploadAddressCardAuthResponseModel>(mAddressCardAuthActivity) {
                    ...
                    public CommonRequestParams createRequestParams() {
                        ...
                        requestParams.addBodyParameter("addressProofType", AddressProofOcrType.DRIVER);// 地址证明的提交类型
                    ...
                    public void handleFailed(int resultCode, String resultMessage) {
                        mAddressCardAuthActivity.showTipDialog(resultMessage);
