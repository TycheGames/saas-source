package com.bigshark.android.vh.authenticate.addresscard.votercard;
 ...
 */
public class AddressCardAuthVoterCardIdVh extends DisplayBaseVh<View, Void> {

    private AddressCardAuthActivity addressCardAuthActivity;
    ...
    public AddressCardAuthVoterCardIdVh(AddressCardAuthActivity activity, MediaStoreCompat mMediaStoreCompat, View root, Callback mCallback) {
        super(activity);
        this.addressCardAuthActivity = activity;
    ...
    protected void bindViews() {
        super.bindViews();
        mFrontRoot = findViewById(R.id.authenticate_address_voterid_front_root);
        mFrontPicImage = findViewById(R.id.authenticate_address_voterid_front_pic);
        mFrontStatusImage = findViewById(R.id.authenticate_address_voterid_front_status);

        mBackRoot = findViewById(R.id.authenticate_address_voterid_back_root);
        mBackPicImage = findViewById(R.id.authenticate_address_voterid_back_pic);
        mBackStatusImage = findViewById(R.id.authenticate_address_voterid_back_status);
    }
    ...
    private void requestPermission(final boolean isFront) {
        ...
        PermissionsUtil.requestPermission(addressCardAuthActivity, new PermissionListener() {
            @Override
            ...
            public void permissionDenied(@NonNull String[] permission) {
                addressCardAuthActivity.showToast(R.string.common_takePhoto_permission_denied_tip);
            }
    ...
    private void pickPhoto(final boolean isFront) {
        ...
        String fileTagName = "address_voterid_" + (isFront ? "front" : "back");
        ...
        File voterCardFile = new File(addressCardAuthActivity.getExternalFilesDir(Environment.DIRECTORY_PICTURES), fileName);

        mMediaStoreCompat.dispatchCaptureIntent(addressCardAuthActivity, voterCardFile, RequestCodeType.VOTER_CARD);
    }


    public boolean onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode != RequestCodeType.VOTER_CARD) {
            return false;
        }

        if (resultCode == Activity.RESULT_OK) {
            final String originalImageFilePath = mMediaStoreCompat.getCurrentPhotoPath();
            String compressedImageFilePath = ImageFileCompressUtils.compressImageFileAndReturnCompressedImageFilePath(addressCardAuthActivity, originalImageFilePath);
            ...
            if (mFrontFilePath != null && mBackFilePath != null) {
                //submit
                HttpSender.post(new CommonResponsePendingCallback<UploadAddressCardAuthResponseModel>(addressCardAuthActivity) {
                    ...
                    public CommonRequestParams createRequestParams() {
                        ...
                        requestParams.addBodyParameter("addressProofType", AddressProofOcrType.VOTER);// 地址证明的提交类型
                    ...
                    public void handleUi(boolean isStart) {
                        ...
                        if (isStart) {
                            MaiDianUploaderUtils.Builder.create(addressCardAuthActivity).setEventName(StringConstant.EVENT_AUTH_AADHAAR_SUBMIT).build();
                        }
                    }

                    @Override
                    public void handleSuccess(UploadAddressCardAuthResponseModel resultData, int resultCode, String resultMessage) {
                        MaiDianUploaderUtils.Builder.create(addressCardAuthActivity).setEventName(StringConstant.EVENT_AUTH_AADHAAR_SUBMIT_SUCCESS).build();
                    ...
                    public void handleFailed(int resultCode, String resultMessage) {
                        addressCardAuthActivity.showTipDialog(resultMessage);
