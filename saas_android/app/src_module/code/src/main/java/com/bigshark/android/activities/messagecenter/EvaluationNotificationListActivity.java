...
import com.bigshark.android.adapters.messagecenter.EvaluationNotificationListAdapter;
...
import com.bigshark.android.http.model.message.NewsCommentModel;
 ...
 * 评价通知
 */
public class EvaluationNotificationListActivity extends DisplayBaseActivity implements MyRecyclerView.LoadMoreListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    private EvaluationNotificationListAdapter mAdapter;
    private int mPage = 1;
    ...
    public void bindViews(Bundle savedInstanceState) {
        ...
        tv_titlebar_title.setText("Evaluation notice");
        initRecyclerView();
    }

    private void initRecyclerView() {
        mRecyclerView.setLayoutManager(new LinearLayoutManager(this));
        mAdapter = new EvaluationNotificationListAdapter(this);
    ...
    public void bindListeners(Bundle savedInstanceState) {
        ...
        mAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                NewsCommentItemModel itemBean = (NewsCommentItemModel) adapter.getData().get(position);
//                if (itemBean.getSex() == UserCenter.instance().getUserGender()) {
//                    ToastUtil.showToast(EvaluationNotificationListActivity.this, "同性之间不能查看");
//                    return;
//                }
                //用户主页
                Intent intent = null;
                if (itemBean.getSex() == 1) {
                    intent = new Intent(EvaluationNotificationListActivity.this, UserMenHomePageActivity.class);
                    intent.putExtra(UserMenHomePageActivity.EXTRA_USER_ID, itemBean.getSend_id());
                } else if (itemBean.getSex() == 2) {
                    intent = new Intent(EvaluationNotificationListActivity.this, UserWomenHomePageActivity.class);
...
//        HttpApi.app().getNewsCommentList(this, requestBean, new HttpCallback<NewsCommentModel>() {
//            @Override
//            public void onSuccess(int code, String message, NewsCommentModel data) {
//                hideProgressBar();
...
//            }
//        });
        HttpSender.post(new CommonResponseCallback<NewsCommentModel>((display())) {
            @Override
            public CommonRequestParams createRequestParams() {
                String url = MmkvGlobal.instance().getCacheUrl(StringConstant.SERVICE_URL_GETNEWSCOMMENTLIST_KEY);
            ...
            @Override
            public void handleSuccess(NewsCommentModel data, int resultCode, String resultMessage) {
                hideProgressBar();
                if (data.getList() != null && data.getList().size() > 0) {
                    canLoading = true;
                    mPage++;
