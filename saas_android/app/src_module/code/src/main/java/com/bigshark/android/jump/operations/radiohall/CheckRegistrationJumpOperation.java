 ...
 * @date 2020/1/7 11:58
 */
public class CheckRegistrationJumpOperation extends JumpOperation<RadioHallJumpModel> {
    static {
        JumpOperationBinder.bind(
                CheckRegistrationJumpOperation.class,
                RadioHallJumpModel.class,
                // 查看报名
                StringConstant.JUMP_RADIO_HALL_CHECK_REGISTRATION
        );
    }

    @Override
    public void start() {
        request.startActivity(new Intent(request.activity(), CheckRegistrationActivity.class));
    }
